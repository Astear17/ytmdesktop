...
function stopDeveloperWatchMake(): void {
  if (!developerWatchMakeChild) return;
  try {
    if (process.platform === "win32") {
      spawn("taskkill", ["/pid", String(developerWatchMakeChild.pid), "/T", "/F"], { shell: true, stdio: "ignore" });
    } else {
      developerWatchMakeChild.kill("SIGTERM");
    }
  } catch (e) {
    log.warn("stopDeveloperWatchMake", e);
  }
  developerWatchMakeChild = null;
}

function syncDeveloperWatchMakeProcess(enable: boolean): void {
  stopDeveloperWatchMake();
  if (!enable) return;
  if (app.isPackaged) {
    log.warn("developer.autoRebuildWindowsExe is ignored in packaged builds");
    return;
  }
  if (process.platform !== "win32") {
    log.warn("developer.autoRebuildWindowsExe is only supported on Windows");
    return;
  }
  log.info(`Starting yarn watch:make in ${process.cwd()} (debounced rebuilds)`);
  developerWatchMakeChild = spawn("yarn", ["watch:make"], {
    cwd: process.cwd(),
    stdio: "inherit",
    shell: true,
    windowsHide: false,
  ...
  developerWatchMakeChild.on("exit", (code, signal) => {
    log.info(`watch:make exited code=${code} signal=${signal ?? ""}`);
    developerWatchMakeChild = null;
  });
  developerWatchMakeChild.on("error", err => {
    log.error("watch:make failed to start — is Yarn on PATH?", err);
    developerWatchMakeChild = null;
  });
}
...
const store = new Conf<StoreSchema>({
  ...
  migrations: {
    ...
    ">=2.0.27": store => {
      if (!store.has("developer.autoRebuildWindowsExe")) {
        store.set("developer.autoRebuildWindowsExe", false);
      }
    },
    ">=2.0.28": store => {
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.integrations.sponsorBlockEnabled) {
    sponsorBlock.provide(store, ytmView);
  }
  ...
  if (newState.developer.autoRebuildWindowsExe !== oldState.developer.autoRebuildWindowsExe) {
    syncDeveloperWatchMakeProcess(newState.developer.autoRebuildWindowsExe);
  }
...
const createOrShowSettingsWindow = (): void => {
  ...
  settingsWindow = new BrowserWindow({
    ...
    minWidth: 640,
    minHeight: 480,
    x: Math.round(mainWindowBounds.x + (mainWindowBounds.width / 2 - 480)),
    y: Math.round(mainWindowBounds.y + (mainWindowBounds.height / 2 - 360)),
    minimizable: false,
    maximizable: true,
    resizable: true,
    frame: true,
    ...
    title: "YouTube Music Desktop Settings",
    webPreferences: {
      sandbox: true,
...
app.on("ready", async () => {
  ...
  ipcMain.on("ytmView:audioData", (event, data) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("ytm:audioData", data);
    }
  });
  ...
  ipcMain.handle("memoryStore:get", (_event, key: string) => {
  ...
  ipcMain.handle("app:getAccentColor", () => {
    if (process.platform === "win32") {
  ...
  ipcMain.handle("app:clearCache", async () => {
    ...
    if (ytmView) {
      await ytmView.webContents.session.clearCache();
    }
    return true;
  });
  ...
  syncDeveloperWatchMakeProcess(store.get("developer").autoRebuildWindowsExe);
});
